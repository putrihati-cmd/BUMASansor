...
class DeliveryRemoteDataSource {
  ...
  Future<List<DeliveryOrderModel>> fetchDeliveryOrders({
    ...
    return data.map((item) => DeliveryOrderModel.fromJson(item as Map<String, dynamic>)).toList();
  }
  ...
  Future<DeliveryOrderModel> createDeliveryOrder(Map<String, dynamic> payload) async {
  ...
  Future<void> assignKurir(String id, String kurirId) async {
    await _dio.put('/delivery-orders/$id/assign-kurir', data: {'kurirId': kurirId});
  }
  ...
  Future<void> confirmDelivery(String id, {String? photoProof}) async {
    await _dio.post('/delivery-orders/$id/confirm', data: {'photoProof': photoProof});
  }
}