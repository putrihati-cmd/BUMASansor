Rencana Aksi Produksi BUMAS Ansor (2–4 Minggu)
Berdasarkan analisis gap terhadap dokumen target (1.md–7.md), berikut adalah rekomendasi prioritas, strategi teknis, dan contoh konfigurasi untuk membawa proyek ke tahap siap rilis produksi.

A. Rencana Prioritas 2–4 Minggu (Item Tugas & Kriteria Terima)
...
4	Testing & Perbaikan Akhir	1. Jalankan E2E testing untuk alur kritis (admin buat produk → kurir antar → warung terima stok → warung jual)
2. Uji deployment dari awal: clone repo → jalankan script deploy → app berfungsi
3. Perbaiki bug dan dokumentasi berdasarkan hasil uji	- Semua skenario kritis berjalan tanpa error
- Deployment script idempotent (bisa dijalankan ulang)<n>- Coverage backend ≥50%, mobile ≥40% (target realistis untuk minggu 4)	Waktu mepet, mungkin perlu prioritaskan bug blocker.
Catatan: Target coverage 80% (backend) dan 70% (mobile) sulit dicapai dalam 4 minggu jika dimulai dari 16% dan 5%. Target di atas adalah progres bertahap. Untuk go-live, coverage 50–60% pada modul inti sudah cukup asalkan dilengkapi dengan integration test yang memadai.
...
Fokus pada alur: login → buat produk → stok masuk → transaksi.
...
Buat satu atau dua skenario end-to-end kritis (misal login → buat pesanan) dengan integration_test.
...
Penutup
Dengan rencana di atas, tim dapat fokus pada hal-hal kritis untuk produksi dalam 2–4 minggu. Prioritaskan deployment backend dan dokumentasi terlebih dahulu agar sistem bisa diakses dan digunakan oleh tim internal. Setelah itu, tingkatkan coverage bertahap dan tambahkan fitur pendukung (print, foto, metrik) sesuai kebutuhan.
...
Sudah ada 31 tests ✓ - fokus tambah edge cases
Test scenario: create order → delivery → payment → stock update
...
Flow: login → scan barcode → add to cart → checkout
...
echo "🚀 Deploying BUMAS Ansor Backend..."
...
2. Clear app data (Settings → Apps → BUMAS → Clear Data)
