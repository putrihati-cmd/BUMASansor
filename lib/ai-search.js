    ...
    try {
        ...
        if (intent.priceRange.min || intent.priceRange.max) {
            where.OR = [
                {
                    AND: [
                        intent.priceRange.min ? { basePrice: { gte: intent.priceRange.min } } : {},
                        intent.priceRange.max ? { basePrice: { lte: intent.priceRange.max } } : {}
                    ]
                },
                {
                    AND: [
                        { salePrice: { not: null } },
                        intent.priceRange.min ? { salePrice: { gte: intent.priceRange.min } } : {},
                        intent.priceRange.max ? { salePrice: { lte: intent.priceRange.max } } : {}
                    ]
                }
            ];
        }
