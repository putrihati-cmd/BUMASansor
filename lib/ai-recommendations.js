...
async function getPersonalizedRecommendations(userId, limit) {
    ...
    const similarUsersPurchases = await prisma.order_items.findMany({
        where: {
            productId: { in: purchasedProducts.map(p => p.productId) },
            order: {
                userId: { not: userId },
                status: { in: ['PAID', 'PROCESSING', 'SHIPPED', 'COMPLETED'] }
            }
    ...
    const similarUserIds = similarUsersPurchases.map(p => p.order.userId);
...
async function getSimilarProducts(productId, limit) {
    ...
    const priceRange = {
        min: Number(sourceProduct.basePrice) * 0.5,
        max: Number(sourceProduct.basePrice) * 1.5
    };
...
Kategori: ${product.categoryName || 'Umum'}
